  ...
  if (TouchInput.isTriggered()) {
    for (i = $gameScreen.maxPictures() - 1; i >= 0; i--) {
      ...
      if (picture) {
        ...
        //liply
        if(window['$dataTrim'] && $dataTrim[picture.name()]){
          var geom = $dataTrim[picture.name()];
          var offsetX = (picture._origin === 0)? 0: -geom.originalSize.width/2;
          var offsetY = (picture._origin === 0)? 0: -geom.originalSize.height/2;

          x = picture.x() + offsetX;
          y = picture.y() + offsetY;
          width = geom.originalSize.width;
          height = geom.originalSize.height;
        }
